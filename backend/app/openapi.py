...
OPERATIONS = {
    ...
    "auth.guest": operation("进入游客试用", public=True, result=USER_CONTEXT,
                            description="创建独立游客会话，默认有效 2 小时，支持续期；重复进入不创建新身份。退出或过期后清理全部游客数据。Cookie 为 HttpOnly，响应包含 guest_session_id 和 guest_expires_at。"),
    "auth.renew_guest_session": operation("续期当前游客会话", result=USER_CONTEXT,
                            description="有效会话从当前时间重新获得 2 小时（由 GUEST_SESSION_SECONDS 配置），保留身份与全部试用记录；过期或退出的会话不可恢复。"),
    ...
    "practice.create_practice": operation(
        "创建练习（学生或游客）", status=201, result=PRACTICE,
